...
function Page() {
    ...
    if (!userInfo) {
      return (
        <div className="flex items-center justify-center h-[70vh] border-1 bg-[#f0f0f0] rounded">
          <p>Please log in to start message</p>
        </div>
      );
    }
    ...
    const sendMessage = async (message: string) => {
      ...
      let biggest = 0;
      messages.forEach(value => {if(value.id > biggest) biggest = value.id})

      setMessages((prev) => [...prev, { id: (biggest + 1), content: message, isSender: true }]);
    };
        ...
        <div className="bg-white rounded-xl w-[20vw] pl-2.5 pr-2.5 pt-2.5 mr-5 shadow-md flex flex-col border border-1">
            <div className="pr-4 h-[80px] mb-[3%]">
              ...
              <div className="h-[50%] relative">
                <SearchUser isSearch={isSearch} setIsSearch={setIsSearch} setListSearchUser={setListSearchedUser} />
              </div>
            </div>
        ...
        {(selectedUser == null || selectedUser == undefined) ?
          <div className="bg-white rounded-xl shadow-md p-6 flex items-center justify-center flex-1 border border-1">
            <div className="text-center">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">No User Selected</h2>
              <p className="text-gray-500">Please select or find a user to view their details.</p>
            </div>
          </div>
         : 
          <div className="bg-white rounded-xl flex flex-col shadow-md flex-1 border border-1">
            ...
            <MessageBox messages={messages} setMessages={setMessages} userId={userInfo.id ?? ""} selectedUser={selectedUser} />
