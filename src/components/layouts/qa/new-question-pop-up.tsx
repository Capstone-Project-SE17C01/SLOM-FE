...
import { usePostQuestionMutation, useUpdateQuestionMutation } from "../../../api/QaApi";
...
export default function NewQuestionPopup({ userInfo, setIsNewQuestion, isUpdateQuestion, question, setIsUpdateQuestion, setQuestion }: Readonly<NewQuestionPopupProps>) {
    ...
    useEffect(() => {
        console.log("existImages updated:", existImages)
    }, [existImages])
    ...
    return (<div>
        ...
        <div className="fixed inset-x-0 top-1/4 z-50 mx-auto w-[48%] p-6 bg-white rounded-lg shadow-lg">
            <div className="pb-3 mb-1 flex w-full justify-between border-b">
                <button onClick={() => { setIsNewQuestion(false); setIsUpdateQuestion(false); setQuestion(undefined); }}>Cancel</button>
            ...
            <div className="flex">
                ...
                <div className="w-[90%]">
                    ...
                    <textarea
                        ...
                        value={newQuestion}
                    />
                    <UploadImage setFiles={setFiles} images={question != undefined ? question.images : []} setExistImages={setExistImages} existImage={existImages} />
                </div>
            </div>
            <div className="w-full flex justify-between mt-5">
                <div className="absolute">
                    <Listbox value={privacy} onChange={setPrivacy}>
                        ...
                        <ListboxOptions className=" mt-1 bg-white rounded-xl shadow-lg">
                            <ListboxOption value="All can view and answer your question" className="px-4 py-2">
                                All
                            </ListboxOption>
                ...
                <div></div>
                <button className="border px-3 py-1 rounded-xl mt-1" onClick={async () => { if (isUpdateQuestion) { await updateQuestion(existImages) } else await postQuestion() }}>{isUpdateQuestion ? "Update" : "Post"}</button>
            </div>
        </div>
    </div>)
}