...
export default function NewAnswer({
  ...
  allQuestion,
  isAdmin,
}: Readonly<NewAnswerProps>) {
  const [newAnswer, setNewAnswer] = useState(
    isUpdateAnswer ? answer?.content || "" : ""
  );
  const [files, setFiles] = useState<File[]>([]);
  const [existImages, setExistImages] = useState<string[] | undefined>(
    isUpdateAnswer ? answer?.images : undefined
  );
  ...
  const postAnswer = async () => {
    ...
    try {
      if (isUpdateAnswer && answer) {
        ...
        await postAnswerAPI(request).then((res) => {
          ...
          if (incomingResult != null) {
            ...
            const lastAnswerAmount = newAnswerAmount?.findLast(
              (val) => val.questionId == lastQuestionId
            );
            ...
            setNewAnswerAmount((prev) => [
              ...(prev ?? []).filter((val) => val.questionId != lastQuestionId),
              newAnswerQuantity,
            ]);
            ...
            if (
              ...
              const updatedQuestions = allQuestion.filter(
        ...
        <div className="space-y-4">
            ...
            <div className="flex-1">
              ...
              >
                <Textarea
                  ...
                  className={cn(
                    "w-full p-3 border-none rounded-b-none focus:ring-0 resize-none",
                    isDarkMode
          ...
          <div className="flex justify-end mt-6">
            <button
              ...
              disabled={
                isSubmitting ||
                (!newAnswer.trim() && !files.length && !existImages?.length)
              }
            >
              <span>
                {isUpdateAnswer
